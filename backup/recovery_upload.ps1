...
$logFile = "D:\erp\backup\logs\upload_recovery_$(Get-Date -Format 'yyyyMMdd').log"
...
# 恢复上传文件
function Restore-Upload {
    param(
        [string]$BackupFile
    )
    Write-Log "开始恢复上传文件: $BackupFile"
    try {
        # 创建临时目录
        ...
        foreach ($item in $backupItems) {
            ...
            Write-Log "已恢复上传文件: $relativePath"
        }
...
# 恢复上传文件
Restore-Upload -BackupFile $latestBackup.FullName
