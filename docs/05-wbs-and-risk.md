...
> **状态**：v1.3(BD-002/007/008 决策:阿里云 + OSS + KMS + 无 SSO,**P3+P4 净省 10 人日**)
...
| P3.5.1 TypeHandler 加密（AES）| P3.5.1 | **1** | - | 后端 | **阿里云 KMS SDK**(BD-008)替代自研,原 3d → 1d |
...
| **P3.7 若依扩展包集成**(2026-04-22 新增,源自 `docs/08` v1.1) | P3.7 | 8 | - | 后端 + 前端 | 4 个能力点 |
| P3.7.1 EasyExcel 消 Python 脚本债(替 20+ `import_*.py`)| P3.7.1 | **3** | - | 后端 | Maven + 改造 `ExcelUtil` + 3 Converter ·**P3.0 即时** |
| P3.7.2 ip2region 登录日志地理字段 | P3.7.2 | **1** | - | 后端 | `AddressUtils` 重写 + ip2region.db |
| P3.7.3 **阿里云 OSS** 样品图/色卡存储(对接 02 AR-005,BD-008 锁定)| P3.7.3 | **1** | - | 后端 | OSS SDK + `FileUploadUtils` 双实现,原 2d → 1d |
| P3.7.4 WebSocket 实时推送(落 `ruoyi-admin/websocket/`)| P3.7.4 | **2** | - | 全栈 | 4 类 + 前端 composable |
| **P3.8 老库数据治理**(2026-04-22 新增,源自 `docs/10`) | P3.8 | 1+? | - | 后端 | 防呆 + 迁移决策 |
| P3.8.1 `ry_20260321.sql` 20 张表补显式 `DEFAULT CHARSET=utf8mb4` | P3.8.1 | **1** | - | 后端 | 防 Pilot DBA 漏配 |
| P3.8.2 CO-006 人脸数据(`sys_user.facedata`)迁入 or 废弃决策 | P3.8.2 | **决策** | - | 合规 + 业务 | 会议决议 + PIPIA(若迁)|
| P3.8.3 **条件触发**:若迁入富泉老库,每表 `ALTER CONVERT TO utf8mb4` | P3.8.3 | **条件** | P3.8.2 | DBA | 批量迁移脚本 |

**P3 阶段汇总**：
- 工作包数：24 + **7**(新增 P3.7 4 + P3.8 3)= **31 个**
- 人日：63 + **6**(原 +9,因 BD-008 阿里云 KMS/OSS 替代自研,P3.5.1 -2 + P3.7.3 -1)= **69 人日**
- 日历周：8 周（人日并行）
...
| 工作包 | 编号 | 工时 | 前置 | 交付物 |
| :-- | :-- | :-- | :-- | :-- |
| **P4.1 合规硬约束** | P4.1 | **15**(BD-005 降级 -10 人日) | P3.5 | - |
...
| P4.1.8 aj-captcha 滑块验证码(与 P4.1.4 同窗口,2026-04-22 新增,源 `docs/08` E3) | P4.1.8 | **2** | P4.1.4 | SPI + Redis 缓存 + 前端 Verifition 组件 merge |
...
**P4 阶段汇总**：**65 人日**(原 75,因 BD-005 PIPL 降级 -10 人日)/ 10 周 / 建议人力 3 后端 + 0.3 DBA(RDS 全托管,降负) + 1 集成专家
...
| 工作包 | 工时 | 交付物 |
| :-- | :-- | :-- |
| P6.0 Docker 一键部署脚手架(2026-04-22 新增,源 `docs/08` E12) | 3 | docker-compose.yml + 4 Dockerfile,绕开 Pilot 老版本 nginx/redis |
| P6.1 数据迁移演练 | 10 | 演练报告（2 次)**先执行 `docs/07` 全量核查** |
...
- **2026-04-22 v1.3**:BD-002 / BD-007 / BD-008 决策吸收:
  - **BD-002 账号密码登录** → P4.1 E4 JustAuth 工作包移除(-5-7 人日)
  - **BD-007 云厂商 = 阿里云** + **BD-008 OSS/KMS/RDS/Tair** → P3.5.1 KMS 3→1d(-2)、P3.7.3 MinIO→OSS 2→1d(-1)
  - **BD-005 PIPL 降级** → P4.1 合规硬约束 25→15 人日(-10)
  - **总净省 10 人日**(原计划 P3+P4 净增 9 → 现净增 -1),同时 DBA 从 0.5→0.2 FTE
  P3 汇总 72→**69 人日**,P4 汇总 75→**65 人日**。总工期不变(9 个月)。
- **2026-04-22 v1.2**:同步 `docs/08`(扩展包)与 `docs/10`(老库)发现,追加 4 段工作包:
  - P3.7 若依扩展包集成(4 项,8 人日):EasyExcel / ip2region / MinIO / WebSocket
  - P3.8 老库数据治理(3 项,1 人日 + 条件触发):CHARSET 防呆 / CO-006 人脸决策 / utf8mb4 迁移
  - P4.1.8 aj-captcha 滑块验证码(2 人日,与 P4.1.4 密码策略同窗口)
  - P6.0 Docker 一键部署脚手架(3 人日,P6.1 数据迁移演练前置,绕开 nginx/redis 老版本问题)
  P3 阶段汇总从 24 包/63 人日 → **31 包/72 人日**。总工期不变(9 个月),人日并行可消化。
