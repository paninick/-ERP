...
> **状态**:v1.3(BD-005 PIPL 降级 + BD-008 KMS 改阿里云)
...
#### 1.1.3 PIPL 第 38 条（数据出境）—— 🟡 **降级条款**(2026-04-22 BD-005)

**场景触发**(v1.2):原假设外贸业务"几乎必然"触发。**经 BD-002/007/008 决策后重新定性**:

| 条件 | 现状 | 触发出境? |
| :-- | :-- | :-: |
| 部署位置 | 阿里云国内 region(BD-007)| ❌ 数据物理不出境 |
| 日方访问 | 日方客户不登 ERP(BD-002)| ❌ 访问层面无跨境 |
| 存储的日方数据 | 仅公司联系人(姓名+邮箱+电话)(最小必要)| ❌ 属"履行合同必需" |
| **综合判定** | PIPL 第 38 条**不触发** | ✅ 可走第 13 条第 2 项豁免 |

**降级后系统要求**(对应豁免路径):
- 🟡 保留 `sys_data_export_log` 表(Pilot 上线前建,防止未来触发重启用)
- 🟡 法务出具一份《对日外贸 ERP 日方联系人信息处理合规说明》(约 0.5 人日)
- 🟡 操作日志 `sys_oper_log` 记录对联系人信息的访问即可,不需专项出境表

**三条合规路径备用**(若未来升级):

| 路径 | 适用条件 | 成本 | 触发升级条件 |
| :-- | :-- | :-- | :-- |
| A. 国家网信办安全评估 | 关键信息基础设施 / 处理 100 万人以上 / 出境累计 1 万人敏感信息 | 极高（6 个月+） | 极罕见,不适用 |
| B. 标准合同备案 | 非 A 情形 | 中(备案 + 合同签署) | 若日方登 ERP 或回传日方 |
| C. 个人信息保护认证 | 特定机构认证 | 中高 | 客户被动要求 |

**原文(v1.0-v1.1)保留给升级参考**:

**系统要求**（对应路径 B,**降级后暂不实施**)：

- 🟡(降级)每笔境外传输需记录:传输对象、目的、字段范围、时间、合法性基础
- 🟡(降级)新建 `sys_data_export_log` 表(预留,未启用)
- 🟡(降级)"数据出境清单"报表供合规审阅
...
| **人脸数据**(legacy `sys_user.facedata`) | **不迁入新库**;若迁,须独立表 `sys_user_biometric` + **阿里云 KMS** 加密 + PIPIA 备案 | **PIPL 第 28 条敏感信息**,单独同意 + 严格访问控制 | 🔴 |
...
- **2026-04-22 v1.3**:BD-005 / BD-008 决策落地:
  - **§1.1.3 PIPL 第 38 条从 🔴 关键条款 → 🟡 降级条款**。三项条件(部署国内 + 日方不访问 + 仅存极少联系人)使 PIPL 第 38 条**不触发**,合规可走第 13 条第 2 项"履行合同必需"豁免。合规工期从 25 人日 → 15 人日(-10 人日)。
  - §6.1 人脸数据行 KMS 明确为"**阿里云 KMS**"(BD-008)。
  原文作为升级参考保留。
