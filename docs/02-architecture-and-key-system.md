...
> **状态**:v1.2(追加 ADR-008 部署假设 · 阿里云 + 国内使用 + 无 SSO + OSS/KMS/RDS/Tair)
...
### ADR-008:部署假设(2026-04-22 新增,BD-007 + BD-008 落地)

- **背景**:BD-007 选阿里云,BD-008 锁定组件套件,BD-002 决议账号密码登录
- **决定**:
  - 云厂商:**阿里云**(region 华东 1 杭州 或 华北 2 北京)
  - 存储:**OSS**(样品图/色卡/合同)替代 MinIO
  - 加密:**阿里云 KMS SDK**(对称 + 非对称)替代自研 AES + KMS
  - 数据库:**RDS for MySQL 5.7 高可用版**(默认 utf8mb4)
  - 缓存:**Redis Tair** 标准版(6.0+)
  - 负载均衡:**SLB / ALB**(内网 + 公网双实例)
  - 鉴权:**Spring Security + JWT**,账号密码登录,不集成 SSO
  - 访问主体:**仅我方员工**,日方客户不登 ERP
- **PIPL 影响**(BD-005 降级):
  - 部署在国内 region → 数据物理不出境
  - 日方不访问 → 访问层面无跨境
  - 仅存日方公司联系人(姓名+邮箱+电话)→ 最小必要数据
  - **PIPL 第 38 条不触发**,合规文档走第 13 条第 2 项"履行合同必需"豁免
- **后果**:
  - DBA 工作量从 0.5 FTE 降至 0.2 FTE(全托管)
  - 运维自建 Nginx / Redis / MinIO 等组件全部规避(docs/09 黑名单扩大)
  - 未来若触发日方访问或数据回传场景,需升级 BD-005(Pilot → 生产前复核)
...
| AR-005 | 大文件（样品图、色卡图）存储 | **阿里云 OSS**(BD-008 v1.2 锁定) | ✅ 已决 |
...
- **2026-04-22 v1.2**:追加 **ADR-008 部署假设**(BD-007 + BD-008 + BD-002 + BD-005 联动):
  - 云厂商:**阿里云**(华东/华北 region)
  - 组件:OSS / KMS / RDS for MySQL 5.7 / Redis Tair / SLB
  - 鉴权:Spring Security + JWT 账号密码登录,不集成 SSO(BD-002)
  - 访问主体:仅我方员工,日方客户不登 ERP
  - PIPL 影响:第 38 条不触发(数据不出境 + 访问层面无跨境 + 最小必要数据)
  §4 AR-005 存储选型从"MinIO / OSS 二选一" → 锁定"阿里云 OSS"(✅ 已决)
  同步:`docs/05 v1.3`(工期 -10 人日)、`docs/03 v1.3`(KMS 简化)、`docs/07 v1.3`(阿里云配置)、`docs/08 v1.2`(E1/E4 状态)、`docs/12 v1.1`(BD-002/005/007/008)。
