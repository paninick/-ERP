...
> **v1.3 更新(BD-008)**:生产 / Pilot **用阿里云 RDS for MySQL 5.7 高可用版**,默认 utf8mb4,免自建。以下核查项仅用于**开发机 / 测试机**;生产走 RDS 控制台确认参数即可。
...
> **v1.3 更新(BD-008)**:生产 / Pilot **用阿里云 Redis Tair**(社区 6.0 标准版),免自建。docs/09 §2.3 的 Windows Redis 3.2.100 问题彻底规避。
...
> **v1.3 更新(BD-008)**:生产 / Pilot **用阿里云 SLB / ALB**(内网 + 公网双实例),免自建 Nginx。docs/09 §2.2 的 Nginx 1.8.1 问题彻底规避。以下条目仅在**本地开发或 ACK 集群内需要 Ingress** 时适用。
...
- **2026-04-22 v1.3**:同步 `docs/12 BD-007 / BD-008` 决策(阿里云全托管):
  - §3.1 MySQL 段前置说明"生产走 RDS for MySQL 5.7 高可用版",核查项仅用于开发/测试机
  - §3.2 Redis 段前置说明"生产走 Redis Tair 标准版 6.0+"
  - §3.3 Nginx 段前置说明"生产走 SLB / ALB",免自建
  - docs/09 黑名单(Nginx 1.8.1 / Redis 3.2.100)彻底规避问题
  - DBA 工作量从 0.5 FTE 降至 0.2 FTE
