...
try:
    ...
    # 获取当前最大ID
    cursor.execute("SELECT MAX(id) FROM t_erp_auxiliary_material")
    ...
    for index, row in df_clean.iterrows():
        try:
            # 检查辅料编号是否已存在
            auxiliary_no = str(row['辅料编号']).strip()
            
            if auxiliary_no:
                cursor.execute(
                    "SELECT id FROM t_erp_auxiliary_material WHERE auxiliary_material_no = %s",
                    (auxiliary_no,)
                )
                if cursor.fetchone():
                    skip_count += 1
