...
public class SampleTechController extends BaseController {
    ...
    @Log(title = "工艺指示书导入", businessType = BusinessType.IMPORT)
    ...
    public AjaxResult importData(MultipartFile file, boolean updateSupport) throws Exception
    {
        ...
        try {
            ExcelUtil<SampleTech> util = new ExcelUtil<SampleTech>(SampleTech.class);
            List<SampleTech> list = util.importExcel(file.getInputStream());
            System.out.println("解析出的数据条数: " + (list != null ? list.size() : 0));
            
            if (list != null && !list.isEmpty()) {
                System.out.println("=== 前5条解析数据 ===");
                for (int i = 0; i < Math.min(5, list.size()); i++) {
                    SampleTech sampleTech = list.get(i);
                    System.out.println("记录[" + i + "]:");
                    System.out.println("  sampleType=" + sampleTech.getSampleType());
                    System.out.println("  sampleId=" + sampleTech.getSampleId());
                    System.out.println("  dueDate=" + sampleTech.getDueDate());
                    System.out.println("  stylePic=" + sampleTech.getStylePic());
                    System.out.println("  tagPic=" + sampleTech.getTagPic());
                }
                System.out.println("=== 前5条解析数据结束 ===");
            }
            
            String message = sampleTechService.importSampleTech(list, updateSupport);
    ...
    {
        ExcelUtil<SampleTech> util = new ExcelUtil<SampleTech>(SampleTech.class);
        util.importTemplateExcel(response, "工艺指示书数据");
    }
}
